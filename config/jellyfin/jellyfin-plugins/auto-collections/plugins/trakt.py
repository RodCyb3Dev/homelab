...
class Trakt(ListScraper):
    ...
    def get_list(list_id, config=None):
        ...
        # Initialize item_types to None - will be set for chart lists or determined from item data
        item_types = None
        ...
        for item_data in items_data:
            if "type" in item_data:
                ...
                # For custom lists, try to infer type from item structure
                if item_types is not None:
                    item = {"media_type": item_types}
                elif "movie" in item_data:
                    item = {"media_type": "movie"}
                elif "show" in item_data:
                    item = {"media_type": "show"}
                else:
                    logger.warning(f"Could not determine media type for item: {item_data}")
                    continue
