...
# Load .env file. 
# Assumes .env from project root is copied to the container's root directory (e.g. /.env)
# and main.py is at a path like /app/main.py.
...
def main(config):
    ...
    if "jellyseerr" in config:
        try:
            ...
            logger.info("Jellyseerr client initialized successfully")
        except Exception as e:
            logger.warning(f"Jellyseerr is not available: {e}. Continuing without Jellyseerr integration.")
            js_client = None
    else:
        js_client = None
